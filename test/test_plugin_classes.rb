...
module FluentTest
  ...
  class FluentTestGenInput < ::Fluent::Plugin::Input
    ...
    def start
      ...
      if @async
        thread_create(:test_in_gen, &method(:emit)) # rubocop:disable Performance/MethodObjectAsBlock
      else
        emit
      end
    end
    ...
    def emit
      @num.times do |i|
        break if @async && !thread_current_running?
  ...
  class FluentTestEmitOutput < ::Fluent::Plugin::Output
    ...
    def write(chunk)
      ...
      array = chunk.map do |time, record|
        [time, record]
      end
  ...
  class FluentTestBuffer < Fluent::Plugin::Buffer
    ...
    def generate_chunk(metadata); end
